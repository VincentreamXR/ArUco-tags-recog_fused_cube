...
def process_frame(frame, dictionary, params, detector, layout, camera_matrix, dist_coeffs, args):
    ...
    lines = [
        "ArUco 5x5_100 rigid object fused pose  dictionary: {}".format(args.dictionary),
...
def run_image(args, dictionary, params, detector, layout, camera_matrix, dist_coeffs):
    ...
    if args.show:
        cv2.imshow("ArUco 5x5_100 Cube Fused Pose", frame)
        cv2.waitKey(0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Detect ArUco DICT_5X5_100 tags id 0-7 and estimate one fused rigid object pose."
    )
    parser.add_argument("--dictionary", default="DICT_5X5_100", choices=sorted(DICTIONARIES))
