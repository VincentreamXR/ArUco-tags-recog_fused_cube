...
def resolve_svg_paths(args, project_dir):
    ...
    svg_paths = []
    subdirs = sorted((path for path in svg_dir.iterdir() if path.is_dir()), key=natural_key)
    for subdir in subdirs:
        ...
        candidates = sorted(subdir.glob("*.svg"), key=natural_key)
        if candidates:
    ...
    flat_svgs = sorted(svg_dir.glob("*.svg"), key=natural_key)
    if flat_svgs:
        preferred_names = ("jumarker_1.svg", f"{svg_dir.name}_1.svg")
        for name in preferred_names:
            preferred = svg_dir / name
            if preferred.exists():
                return [preferred.resolve()]
        return [flat_svgs[0].resolve()]
...
def build_command(args):
    ...
    if args.autofocus is not None:
        cmd.extend(["--autofocus", str(args.autofocus)])
...
def main():
    ...
    parser.add_argument(
        "--project",
        default="/home/zsyy/桌面/JuMarker",
        help="JuMarker project root directory. Default: /home/zsyy/桌面/JuMarker",
    )
    parser.add_argument(
        "--build",
        default="build",
    ...
    parser.add_argument(
        "--svg-dir",
        default="output_chimeta",
        help=(
            "Directory containing generated Chimeta/JuMarker SVG tags. "
            "Flat directories are supported; by default output_chimeta/jumarker_1.svg is used as the template."
        ),
    )
    parser.add_argument(
        "--id-bits",
        type=int,
        default=5,
        help="ID bits used when generating Chimeta markers. For IDs 1-32, use 5.",
    )
    parser.add_argument(
        "--camera",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=(0, 1),
        default=1,
        help="Enable or disable camera autofocus. Default: 1.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
