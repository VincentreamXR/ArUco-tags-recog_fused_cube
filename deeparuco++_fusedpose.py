...
def create_deeparuco_bit_decoder(aruco_module):
    ...
    for marker_bits in aruco_module.ids_as_bits:
        ...
        templates.append([np.rot90(canonical, rotation_index).astype(np.float32) for rotation_index in range(4)])
...
def create_deeparuco_backend_decoder(codebook, dictionary_name, aruco_module):
    if codebook == "mip36h12":
        return create_deeparuco_bit_decoder(aruco_module), "MIP36h12"
    if codebook == "opencv":
        return create_aruco_bit_decoder(dictionary_name), dictionary_name
    raise ValueError("Unsupported DeepArUco++ codebook: {}".format(codebook))


def load_deeparuco_backend(repo, detector_name, regressor_name, dictionary_name, codebook="mip36h12"):
    ...
    find_id, codebook_display_name = create_deeparuco_backend_decoder(codebook, dictionary_name, deeparuco_aruco)
    ...
    return {
        ...
        "codebook": codebook,
        "codebook_display_name": codebook_display_name,
        "find_id": find_id,
...
def draw_render_state(
    ...
    codebook_display = getattr(args, "deeparuco_codebook", "opencv") if backend_name == "deeparuco" else "opencv"
    lines = [
        "DeepArUco++ rigid object fused pose  backend: {}  codebook: {}  dictionary: {}  video_fps={:.1f} process_fps={:.1f} display_fps={:.1f} tag_detect={:.1f}ms resolution={}".format(
...
def parse_args():
    ...
    parser.add_argument(
        "--deeparuco-codebook",
        choices=("mip36h12", "opencv"),
        default="mip36h12",
        help="Decode DeepArUco++ marker bits with the repository MIP36h12 codebook or an OpenCV ArUco dictionary. Default: mip36h12.",
    )
    parser.add_argument(
        "--deeparuco-threshold",
        type=float,
