...
def create_detector_parameters(args):
    ...
    if hasattr(params, "useAruco3Detection"):
        params.useAruco3Detection = not args.disable_aruco3
    if hasattr(params, "minSideLengthCanonicalImg"):
        params.minSideLengthCanonicalImg = args.min_side_length_canonical_img
    if hasattr(params, "minMarkerLengthRatioOriginalImg"):
        params.minMarkerLengthRatioOriginalImg = args.min_marker_length_ratio_original_img
    return params
...
def draw_text_panel(image, lines):
    ...
    width = min(image.shape[1] - 24, 900)
...
def annotate_frame(frame, dictionary, params, detector, args, camera_matrix=None, dist_coeffs=None):
    ...
    if marker_count > 0:
        ...
        ids_text = marker_ids.flatten().astype(int).tolist()
    ...
    lines = [
        "ArUco3: {}  dictionary: {}".format("on" if aruco3_enabled(params) else "off", args.dictionary),
        "markers: {}  ids: {}  rejected: {}".format(marker_count, ids_text[:12], len(rejected)),
        "resolution: {}x{}".format(frame.shape[1], frame.shape[0]),
    ]
    draw_text_panel(frame, lines)
    return marker_count, ids_text, len(rejected)


def aruco3_enabled(params):
    return bool(getattr(params, "useAruco3Detection", False))


def run_image(args, dictionary, params, detector, camera_matrix, dist_coeffs):
    image_path = Path(args.image).expanduser()
    frame = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
    if frame is None:
        raise RuntimeError("Cannot read image: {}".format(image_path))

    count, ids, rejected_count = annotate_frame(frame, dictionary, params, detector, args, camera_matrix, dist_coeffs)
    print("image: {}".format(image_path), flush=True)
    print("markers: {} ids: {} rejected: {}".format(count, ids, rejected_count), flush=True)
    ...
    if args.show:
        cv2.imshow("ArUco3 Detection", frame)
        cv2.waitKey(0)
        cv2.destroyAllWindows()


def run_camera(args, dictionary, params, detector, camera_matrix, dist_coeffs):
    ...
    print("Camera: {} requested={}x{} autofocus={}".format(args.camera, args.width, args.height, args.autofocus), flush=True)
    print("ArUco3: {} dictionary={} marker_length={}".format(aruco3_enabled(params), args.dictionary, args.marker_length), flush=True)
    ...
    while True:
        ...
        annotate_frame(frame, dictionary, params, detector, args, camera_matrix, dist_coeffs)
        cv2.imshow("ArUco3 Detection", frame)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Detect ArUco markers with OpenCV ArUco3 detection.")
    ...
    parser.add_argument("--disable-aruco3", action="store_true", help="Disable OpenCV useAruco3Detection.")
    ...
    parser.add_argument("--min-side-length-canonical-img", type=int, default=32)
    parser.add_argument("--min-marker-length-ratio-original-img", type=float, default=0.0)
...
def main():
    ...
    dist_coeffs = None
    if args.calibration:
        camera_matrix, dist_coeffs = read_calibration(Path(args.calibration).expanduser())

    if args.image:
        run_image(args, dictionary, params, detector, camera_matrix, dist_coeffs)
    else:
        run_camera(args, dictionary, params, detector, camera_matrix, dist_coeffs)
